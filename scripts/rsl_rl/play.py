...
# # Isaac Lab >= 0.50.7 moved pretrained checkpoint utilities to isaaclab_rl.utils.
# try:
#     from isaaclab_rl.utils.pretrained_checkpoint import get_published_pretrained_checkpoint
# except ModuleNotFoundError:
#     # Backward compatibility for older Isaac Lab releases.
#     from isaaclab.utils.pretrained_checkpoint import get_published_pretrained_checkpoint
...
#     # handle deprecated configurations (e.g. old `policy` -> new `actor`/`critic` for rsl-rl >= 4.0.0)
...
def main():
    ...
    if pkg_version.parse(installed_version) >= pkg_version.parse("4.0.0"):
        # rsl-rl >= 4.0.0: use the runner's built-in export methods
        runner.export_policy_to_jit(path=export_model_dir, filename="policy.pt")
        runner.export_policy_to_onnx(path=export_model_dir, filename="policy.onnx")
    else:
        # rsl-rl < 4.0.0: manually extract the combined actor-critic module and export
